// 204101070_yes_no.cpp : Defines the entry point for the console application.
//
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	system("Recording_Module.exe 3 input_file.wav input_file.txt");  //Using realtime recording
	ifstream yesno;  //Creating an object for reading the file
	yesno.open("input_file.txt");  //Opening the .txt file in which the voice(yes/no) is stored
	...
	for(long long i = 0; i<size-15; i++){  //For iterating over the complete vectors
		if(ste[i] > 1000000){  //STE value is compared first. If it is less than 1000000 then it must be either silence or noise. Hence STE should be high
			...
			else if(zcr[i+15] < 30){  //If ZCR is less than 100 that means either it is a silence or a vowel. Therefore it should output No
			...
			continue;  //If STE is less than 1000000 then it should be silence or noise hence increment the value of i by 1
		}
	}
